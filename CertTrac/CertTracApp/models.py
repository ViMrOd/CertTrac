...
class TotalTrainingSession(models.Model):
    '''
    Model representing sessions that contribute to the total training time for courses.

    Fields:
        - course: A foreign key to the Course model, indicating the course related to this session.
        - date: The date for which the session is logged (max length: 40 characters).
        - training_time: The amount of training time logged during this session (hours and minutes).

    This model serves as a record of sessions that contribute to the total training time associated with specific courses. The "training_time" field represents the amount of training time logged during each session. These sessions collectively contribute to the overall total training time for a course.
    '''
    course = models.CharField(max_length = 40)
    date = models.CharField(max_length=40)
    training_time = models.DecimalField(max_digits=5, decimal_places=2)
